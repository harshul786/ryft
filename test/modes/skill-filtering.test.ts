...
test('mode-skills - getModeSkills loads skills for coder mode', async () => {
  ...
  assert.ok(skills.some(s => s.name === 'edit'), 'Should include edit skill');
  assert.ok(skills.some(s => s.name === 'compact'), 'Should include shared compact skill');
});
...
test('mode-skills - getModeSkills loads skills for browser-surff mode', async () => {
  ...
  assert.ok(skills.some(s => s.name === 'browser'), 'Should include browser skill');
  assert.ok(skills.some(s => s.name === 'compact'), 'Should include shared compact skill');
});
...
test('mode-skills - getModeSkills loads skills for debugger mode', async () => {
  ...
  assert.ok(skills.some(s => s.name === 'troubleshoot'), 'Should include troubleshoot skill');
  assert.ok(skills.some(s => s.name === 'compact'), 'Should include shared compact skill');
});
...
test('mode-skills - database getSkillRequiredTools returns tool requirements', () => {
  ...
  assert.deepEqual(browserTools, ['browser'], 'browser should require browser tool');
  
  const troubleshootTools = getSkillRequiredTools('troubleshoot');
  assert.deepEqual(troubleshootTools, ['logs', 'bash'], 'troubleshoot should require logs and bash');
...
test('mode-skills - database skill modes are correctly set', () => {
  ...
  assert.deepEqual(browserModes, ['browser-surff'], 'browser only available in browser-surff');
  
  const troubleshootModes = getSkillMetadata('troubleshoot')?.modes;
  assert.deepEqual(troubleshootModes, ['debugger'], 'troubleshoot only available in debugger');
  
  const compactModes = getSkillMetadata('compact')?.modes;
  assert.deepEqual(compactModes, ['coder', 'browser-surff', 'debugger'], 'compact available in all modes');
});
...
test('mode-skills - coder and debugger modes have different skills', async () => {
  ...
  assert.ok(debuggerNames.has('troubleshoot'), 'Debugger should have troubleshoot');
  assert.ok(!coderNames.has('troubleshoot'), 'Coder should not have troubleshoot');
});
...
test('mode-skills - all modes can load skills without errors', async () => {
  ...
  for (const mode of modes) {
    try {
      const skills = await getModeSkills(mode);
      assert.ok(Array.isArray(skills), `Mode ${mode.name} should return array of skills`);
    } catch (error) {
...
test('GAP FIX #1/#8 - clearSkillsDbCache invalidates cache', async () => {
  ...
  assert.deepEqual(skills1.length, skills2.length, 'Should have same skill count');
});
...
test('GAP FIX #2 - getAllSkillsAcrossModes returns all unique skills', () => {
  ...
  assert.ok(skillNames.includes('troubleshoot'), 'Should include troubleshoot skill');
...
test('GAP FIX #3 - getSkillsRequiringPermission filters by mode', () => {
  ...
  assert.ok(Array.isArray(coderPermissions), 'Should return array for coder mode');
});
...
test('GAP FIX #9 - getModesContainingSkill returns all modes for skill', () => {
  ...
  assert.deepEqual(nonExistentModes, [], 'nonexistent skill should have no modes');
});
...
test('GAP FIX #10 - getAllSkillMetadata returns all skills with metadata', () => {
  ...
  assert.ok(metadata.every(s => s.name && s.modes && s.requiredTools), 'All should have required fields');
});
...
test('GAP FIX #10 - getAllSkillsRequiringTool returns skills needing tool', () => {
  ...
  assert.ok(editorSkillNames.includes('edit'), 'edit skill requires editor tool');

  const browserSkills = getAllSkillsRequiringTool('browser');
  const browserSkillNames = browserSkills.map(s => s.name);

  assert.ok(browserSkillNames.includes('browser'), 'browser skill requires browser tool');
...
test('GAP FIX #11 - validateSkillsDatabase checks entire DB', () => {
  ...
  assert.ok(validation.valid || validation.errors.length > 0, 'Should have validation result');
...
test('GAP FIX #6 - disableSkill marks skill as disabled', () => {
  ...
  assert.ok(state.disabledSkills['edit'].modes.includes('coder'), 'edit should be disabled in coder');
  assert.ok(isSkillDisabledInMode('edit', 'coder'), 'Verification: edit should be disabled');
});
...
test('GAP FIX #6 - disableSkill with multiple modes', () => {
  ...
  assert.ok(state.disabledSkills['compact'].modes.includes('coder'), 'compact disabled in coder');
  assert.ok(state.disabledSkills['compact'].modes.includes('debugger'), 'compact disabled in debugger');
  assert.ok(!state.disabledSkills['compact'].modes.includes('browser-surff'), 'compact not disabled in browser-surff');
});
...
test('GAP FIX #6 - enableSkill removes skill from disabled', () => {
  ...
  assert.ok(!isSkillDisabledInMode('edit', 'coder'), 'edit should be enabled after enableSkill');
  assert.ok(!state.disabledSkills['edit'], 'edit should be removed from disabled state');
});
...
test('GAP FIX #6 - resetSkillsState clears all disabled skills', () => {
  ...
  assert.deepEqual(Object.keys(state.disabledSkills).length > 0, true, 'Should have disabled skills');

  resetSkillsState();
  
  state = getSkillsState();
  assert.deepEqual(state.disabledSkills, {}, 'disabledSkills should be empty after reset');
});
...
test('GAP FIX #12 - getAvailableSchemaVersions lists all versions', () => {
  ...
  assert.ok(Object.keys(versions).every(v => /^\d+\.\d+\.\d+$/.test(v)), 'All should be semver');
});
...
test('GAP FIX #7 - resolveSkillPath uses mapping for known skills', () => {
  ...
  assert.ok(editPath.includes('packs/coder/skills/edit'), 'Should resolve edit path');
...
test('GAP FIX #7 - resolveSkillPath uses fallback for unknown skills', () => {
  ...
  assert.ok(customPath.includes('custom/path/SKILL.md'), 'Should use fallback path');
});

test('GAP FIX #7 - resolveSkillPath returns empty string for unmapped skill', () => {
  const path = resolveSkillPath('unmapped-skill');

  assert.equal(path, '', 'Should return empty string for unmapped skill without fallback');
});
