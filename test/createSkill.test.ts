...
const round1Problem =
  "Automate cherry-picking merged PRs to release branches so we can quickly backport hotfixes without manual rebasing";
const round2Scope =
  "Works on git repositories with bash commands. Needs git for checking commits and handling conflicts. File patterns: *.sh, release branches with naming like v1.*, stable, etc.";
const round3Effort =
  "High - involves multiple steps with conflict detection and resolution";
...
const ts_problem =
  "Convert CommonJS requires to ES6 imports in TypeScript files to modernize codebase";
const ts_scope =
  "Operates on *.ts files in src directory. Needs TypeScript compiler and file editing capabilities.";
...
const db_problem =
  "Analyze database schemas and generate migration SQL for schema updates";
const db_scope =
  "Works with PostgreSQL and MySQL databases. Needs database CLI tools and SQL generation. Files: *.sql, migrations/";
...
const toolTestText =
  "This skill uses bash to execute git commands and read/write files. It may need network access via curl for API calls.";
const detectedTools = (
  await import("../src/commands/createSkill.ts")
).parseToolsFromText(toolTestText);
...
console.log(
  "🎉 All tests passed! Skill creation feature is ready for integration.",
);
