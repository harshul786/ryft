...
import { SkillRegistry, resetGlobalRegistry, getGlobalSkillRegistry } from '../../src/skills/registry.ts';
...
test('SkillRegistry - size() returns correct count', async () => {
  ...
  await registry.register({ name: 'skill1', description: 'D1', file: '/tmp/1.md' }, 'project');
  assert.equal(registry.size(), 1);

  await registry.register({ name: 'skill2', description: 'D2', file: '/tmp/2.md' }, 'user');
...
test('SkillRegistry - getBySource filters by source', async () => {
  ...
  await registry.register({ name: 'project-skill', description: 'P', file: '/tmp/p.md' }, 'project');
  await registry.register({ name: 'user-skill', description: 'U', file: '/tmp/u.md' }, 'user');
  await registry.register({ name: 'bundled-skill', description: 'B', file: '/tmp/b.md' }, 'bundled');
...
test('SkillRegistry - onLoad callback is called on emitLoad', (t, done) => {
  ...
  registry.register({ name: 'test', description: 'Test', file: '/tmp/test.md' }, 'project').then(() => {
    registry.emitLoad();
...
test('SkillRegistry - resetGlobalRegistry clears everything', async () => {
  ...
  await registry.register({ name: 'test', description: 'Test', file: '/tmp/test.md' }, 'project');
  assert.equal(registry.size(), 1);
