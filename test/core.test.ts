...
import { defaultModelOption, formatModelPicker, listModelOptions, resolveModelOption } from '../src/models/catalog.ts';
...
import { buildRequestBody, resolveRouteForModel } from '../src/runtime/router.ts';
...
test('supports multi-mode selection', () => {
  ...
  assert.deepEqual(modes.map(mode => mode.name), ['coder', 'debugger']);
});

test('resolves memory modes', () => {
  assert.equal(defaultMemoryMode().name, 'claude-like');
  assert.equal(resolveMemoryMode('session').name, 'session');
});

test('caps prompt size roughly', () => {
  const capped = applyTokenCap('one two three four five six seven eight nine ten eleven twelve', 3);
  assert.ok(capped.length > 0);
});

test('loads directory-backed skills', async () => {
  const skills = await loadSkillsForModes(resolveModes(['coder', 'browser-surff']));
...
test('builds a routed request body with the selected model', () => {
  const route = resolveRouteForModel(
    'local/gpt-4.1-mini',
    listModelOptions(),
    {
      default: {
...
test('formats browser tab details and DevTools URLs', () => {
  ...
  assert.match(buildDevToolsUrl(9222, 'tab-1'), /devtools:\/\/devtools\/bundled\/inspector\.html/);
...
test('persists claude-like memory and keeps session memory in-process', async () => {
  ...
  const memoryPath = path.join(tempHome, '.ryft', 'memory', 'claude-like');
  const files = await import('node:fs/promises').then(fs => fs.readdir(memoryPath));
  assert.equal(files.length, 1);
  const fileContent = await readFile(path.join(memoryPath, files[0]!), 'utf8');
  assert.match(fileContent, /remember this/);
  assert.match(await loadMemoryContent('claude-like', { cwd: tempWorkspace, homeDir: tempHome }), /remember this/);
  assert.match(await loadMemoryContent('claude-like', { cwd: tempWorkspace, homeDir: tempHome }), /Recent Notes/);
  ...
  assert.match(await loadMemoryContent('hierarchy', { cwd: tempWorkspace, homeDir: tempHome }), /tree note/);
  ...
  const hierarchyPrompt = await promptWithSession(hierarchySession, 'open the workspace');
...
test('compacts history by calling the model and preserving the tail', async () => {
  ...
  const upstream = createServer(async (req, res) => {
    ...
    res.writeHead(200, { 'content-type': 'text/event-stream' });
    res.end('data: {"choices":[{"delta":{"content":"# Summary\\n- preserved"}}]}\n\ndata: [DONE]\n\n');
  });
