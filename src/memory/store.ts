...
function claudeLikePath(context: MemoryContext): string {
  return path.join(resolveHomeDir(context), '.ryft', 'memory', 'claude-like', `${workspaceKey(resolveCwd(context))}.md`);
}
...
async function buildHierarchyTree(rootDir: string, maxDepth = 2, depth = 0): Promise<string[]> {
  if (depth > maxDepth) {
    return [];
  }
  ...
  try {
    ...
    for (const entry of entries.slice(0, 60)) {
      ...
      if (entry.isDirectory) {
        lines.push(...await buildHierarchyTree(path.join(rootDir, entry.name), maxDepth, depth + 1));
      }
    }
    return lines;
  } catch {
...
export async function loadMemoryContent(
  ...
  if (memoryMode === 'session') {
    return trimToMaxChars(context.sessionSnapshot ?? '', MAX_MEMORY_SNIPPET_CHARS);
  }

  const filePath = memoryMode === 'claude-like' ? claudeLikePath(context) : hierarchyPath(context);
...
export async function recordMemoryTurn(
  ...
  if (!existing.trim()) {
    existing = memoryMode === 'claude-like' ? buildClaudelikeTemplate() : await buildHierarchyTemplate(cwd);
  }
