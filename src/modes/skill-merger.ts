...
function loadSkillsDb(): SkillsDb {
  ...
  try {
    ...
    if (!DB_SCHEMA_VERSIONS[currentVersion as keyof typeof DB_SCHEMA_VERSIONS]) {
      console.warn(`Unknown database version: ${currentVersion}. Attempting fallback.`);
    }
 ...
 */
export function getSkillMetadata(skillIdOrName: string | number): SkillDbEntry | undefined {
  const db = loadSkillsDb();
 ...
 */
export function validateSkillDefinition(skill: SkillDbEntry): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
 ...
 */
export function disableSkill(skillId: string, modes: string | string[]): SkillsState {
  const state = loadSkillsState();
 ...
 * Returns updated state
 */
export function enableSkill(skillId: string, modes: string | string[]): SkillsState {
  const state = loadSkillsState();
  ...
  state.disabledSkills[skillId].modes = state.disabledSkills[skillId].modes.filter(
    m => !modesToEnable.includes(m)
  );
...
export function getDisabledSkillsForMode(mode: string): string[] {
  const state = loadSkillsState();
  return Object.keys(state.disabledSkills).filter(
    skillId => state.disabledSkills[skillId].modes.includes(mode)
  );
}
 ...
 */
export function resolveSkillPath(skillId: string, fallbackPath?: string): string {
 ...
 */
export function getModuleToolPolicy(mode: string): { enabledTools?: string[]; disabledTools?: string[] } {
 ...
 */
export function isSkillToolCompatibleWithMode(skillId: string, mode: string): boolean {
 ...
 */
export function filterSkillsByToolPolicy(skills: Skill[], mode: string): Skill[] {
